...
class Process
  ...
  @@pending = {} of Int32 => LibC::DWORD
  @@waiting = {} of Int32 => Channel(Int32)
  ...
  protected def self.pid_system : LibC::DWORD
  ...
  def exec_internal(command : String, args : (Array | Tuple)?, env : Env?, clear_env : Bool, input : IO::FileDescriptor, output : IO::FileDescriptor, error : IO::FileDescriptor, chdir : String?)
    ...
    if ret != 0
      ...
      @handle = wait_handle
      @pi.dwProcessId.to_i32
    else
      raise WinError.new("CreateProcessW")
    end
  end

  protected def self.on_exited(context : Void*, isTimeOut : Bool)
  ...
  protected def self.create_env_block(env : Env?, clear_env : Bool)
    ...
    begin
      Crystal::System::Env.parse_env_block(env_block) { |key, val|
        final_env[key] = val
      }
    ensure
  ...
  protected def self.is_main_window(handle : LibC::HWND) : Bool
    return LibC.GetWindow(handle, LibC::GW_OWNER).address == 0 && LibC.IsWindowVisible(handle) == 1
  end
  ...
  private def to_windows_path(path : String) : LibC::LPWSTR
    return path.check_no_null_byte.to_utf16.to_unsafe
  end
